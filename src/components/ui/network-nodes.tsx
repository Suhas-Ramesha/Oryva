...
/**
 * Animated node graph: edges trace themselves in one by one, then nodes pop and
 * pulse. Purely decorative background flourish for the hero.
 */
    ...
    >
      {EDGES.map(([a, b], i) => (
        <motion.line
          ...
          transition={{ duration: 1.2, delay: 0.3 + i * 0.03, ease: "easeOut" }}
        />
      ))}
      {NODES.map((node, i) => (
        <motion.circle
